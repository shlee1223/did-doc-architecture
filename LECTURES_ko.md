# OmniOne Open DID 강의 시리즈
> OmniOne Open DID를 쉽게 이해하고 직접 실행해볼 수 있도록 강의 시리즈를 준비했습니다.<br>
> 기초 개념부터 서버 설치 실습까지, Open DID를 단계별로 학습할 수 있습니다.

---

목차
--
- [OmniOne Open DID 강의 시리즈 : 입문자용](LECTURES_INTRO_ko.md)
  - [1강. DID 기초 소개](LECTURES_INTRO_ko.md#1강-did-기초-소개)
  - [2강. Open DID 플랫폼 구성요소](LECTURES_INTRO_ko.md#2강-open-did-플랫폼-구성요소)
  - [3강. Open DID 신뢰환경](LECTURES_INTRO_ko.md#3강-open-did-신뢰환경)
  - [4강. 사용자 등록](LECTURES_INTRO_ko.md#4강-사용자-등록)
  - [5강. VC 발급](LECTURES_INTRO_ko.md#5강-vc-발급)
  - [6강. VP 제출](LECTURES_INTRO_ko.md#6강-vp-제출)
  - [부록. Open DID GitHub Repository 구성 및 소개](LECTURES_INTRO_ko.md#부록-open-did-github-repository-구성-및-소개)
- [OmniOne Open DID 강의 시리즈 : 실습 튜토리얼](LECTURES_TUTORIAL_ko.md)
  - [1강. Open DID 실습 교육 개요](LECTURES_TUTORIAL_ko.md#1강-open-did-실습-교육-개요)
  - [2-1강. Open DID 서버 설치 (Orchestrator)](LECTURES_TUTORIAL_ko.md#2-1강-open-did-서버-설치-orchestrator)
  - [2-2강. Open DID 서버 설치 (Gradle)](LECTURES_TUTORIAL_ko.md#2-2강-open-did-서버-설치-gradle)
  - [2-3강. Open DID 서버 설치 (IDE)](LECTURES_TUTORIAL_ko.md#2-3강-open-did-서버-설치-ide)
  - [2-4강. Open DID 서버 설치 (Docker)](LECTURES_TUTORIAL_ko.md#2-4강-open-did-서버-설치-docker)
- [더 많은 강의가 곧 공개됩니다...]
